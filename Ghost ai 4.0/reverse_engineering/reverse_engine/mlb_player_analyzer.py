...
class MLBPlayerAnalyzer:
    def __init__(self, 
                 odds_data_path='data/historical_odds/baseball_mlb',
    ...
    def load_all_data(self):
        """Load all MLB data from multiple sources"""
        print("Loading MLB historical data...")
    ...
    def load_historical_odds(self):
        ...
        if not os.path.exists(self.odds_data_path):
            print(f"Odds data path not found: {self.odds_data_path}")
            return
        ...
        for filename in os.listdir(self.player_tags_path):
            if filename.endswith('.json'):
                ...
                try:
                    ...
                    sport = player_data.get('sport', 'mlb')
                    
                    # Only process MLB players
                    if sport.lower() == 'mlb' and player_name:
    ...
    def load_historical_props(self):
        ...
        try:
            ...
            # Process historical props data - it's a dictionary, not a list
            for player_name, player_props in historical_props.items():
                # Only process MLB players
                if player_name in self.players_data:
                    if 'historical_props' not in self.players_data[player_name]:
                        self.players_data[player_name]['historical_props'] = []
                    
                    # Add all prop records for this player
                    for prop_type, prop_records in player_props.items():
                        for prop_record in prop_records:
                            # Add player name and sport to each record for consistency
                            prop_record['player_name'] = player_name
                            prop_record['sport'] = 'mlb'
    ...
    def generate_comprehensive_report(self):
        ...
        print("\n=== MLB COMPREHENSIVE ANALYSIS REPORT ===\n")
...
if __name__ == "__main__":
    analyzer = MLBPlayerAnalyzer()
    report = analyzer.generate_comprehensive_report()
    
    print("\n=== ANALYSIS COMPLETE ===")
    print("The analyzer has processed all available MLB data and generated comprehensive insights.")
