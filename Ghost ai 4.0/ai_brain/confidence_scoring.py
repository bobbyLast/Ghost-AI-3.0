...
class ConfidenceScorer:
    ...
    def _score_single_prop(self, prop: Dict[str, Any]) -> float:
        ...
        base_conf = 0.5
        prop_type = prop.get('prop_type', '').lower() or prop.get('prop', '').lower()
        # Special handling for rare props
        if 'home_run' in prop_type or 'homerun' in prop_type:
            return 0.20  # Cap at 20%
        if 'hrr' in prop_type or 'hits_runs_rbis' in prop_type:
            # HRR is safer but still not super high
