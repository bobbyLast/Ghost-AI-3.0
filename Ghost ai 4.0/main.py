...
def ai_analyze_market_conditions():
    ...
    try:
        ...
        market_analysis = ai_brain.ghost_brain.get_recent_performance()
...
def ai_make_intelligent_decisions():
    ...
    try:
        ...
        if recent_performance.get('tickets_posted_today', 0) < 5:
            decision = {'action': 'generate_tickets', 'reasoning': 'Need more tickets for today'}
        else:
            decision = {'action': 'analyze_market', 'reasoning': 'Sufficient tickets posted, analyzing market'}
...
def ai_fetch_all_props():
    ...
    try:
        ...
        logger.info("🧠 AI: Fetching Tennis props...")
        import run_tennis_fetch
...
def ai_post_tickets_intelligently():
    ...
    try:
        # Remove all direct Path('ghost_ai_core_memory/tickets/generated') and file writes for tickets
        # Ensure all ticket saving and posting uses hook_ticket_generation
...
def ai_learn_from_results():
    ...
    try:
        ...
        from ai_brain.ai_brain import AIBrain
        ai_brain = AIBrain()
        ai_brain.ghost_brain.think("Learning from results and adapting strategy")
...
def ai_adapt_strategy():
    ...
    try:
        ...
        from ai_brain.ai_brain import AIBrain
        ai_brain = AIBrain()
        ai_brain.ghost_brain.think("Adapting strategy based on performance")
...
def fetch_all_props_and_generate_tickets():
    logger.info("AI: Fetching all sports props (MLB, WNBA, Tennis)...")
    try:
        ...
        logger.info("AI: All sports props fetched successfully")
        
        # Generate tickets using all sports props
        logger.info("AI: Generating tickets using all sports props...")
        from ai_brain.ticket_generator import TicketGenerator
        generator = TicketGenerator(base_dir=Path.cwd())
        new_tickets = generator.generate_tickets(num_tickets=10)  # Generate 10 tickets using all sports props
        
        if new_tickets:
            logger.info(f"AI: Generated {len(new_tickets)} new tickets from all sports")
            
            # Save new tickets using the unified ticket manager hook
            saved_tickets = hook_ticket_generation(new_tickets)
            logger.info(f"AI: Saved {len(saved_tickets)} tickets to unified storage")
            ...
            for ticket in saved_tickets:
                ...
                if (ticket.get('props') and isinstance(ticket['props'], list) and len(ticket['props']) > 0) or (ticket.get('legs') and isinstance(ticket['legs'], list) and len(ticket['legs']) > 0):
                    ...
                    response = requests.post(webhook_url, json=payload)
                    if response.status_code == 204:
                        logger.info(f"AI: Posted new ticket to {'moneyline' if is_moneyline else 'ticket'} webhook")
                        posted_count += 1
                    else:
                        logger.warning(f"AI: Failed to post new ticket")
                else:
                    logger.info(f"AI: Skipped empty or malformed new ticket")
            if posted_count > 0:
                logger.info(f"AI: Successfully posted {posted_count} new tickets.")
            else:
                logger.info(f"AI: No new tickets were posted.")
        else:
            logger.warning(f"AI: No new tickets generated")
            
    except Exception as e:
        logger.error(f"AI: Ticket generation/posting failed - {e}")
...
def ai_decide_sleep_wake():
    ...
    try:
        ...
        # Factor 3: Recent performance (if AI is doing poorly, sleep)
        if recent_performance.get('win_rate', 0) < 0.3:
            should_sleep = True
            reasoning += " - Low performance, need rest"
