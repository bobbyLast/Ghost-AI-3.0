...
from core.tennis_api_client import get_fixtures, get_odds
...
class TicketGenerator:
    ...
    def _load_all_fetched_props(self) -> List[Dict]:
        """Load all props from fetched OddsAPI files for MLB and WNBA."""
        all_props = []
        
        # Helper to normalize OddsAPI prop
        def normalize_oddsapi_prop(game, market, outcome, sport):
            return {
                'sport': sport,
                ...
                'odds': outcome.get('price', 1.9),
                'reasoning': f"Auto-extracted from OddsAPI: {market.get('key', '')} for {outcome.get('name', '')}"
            }
            ...
            for prop_file in mlb_files:
                try:
                    with open(prop_file, 'r') as f:
                        game = json.load(f)
                        logger.info(f"Processing MLB file: {prop_file.name}, data keys: {list(game.keys())}")
                        # OddsAPI format: has 'bookmakers' key
                        if isinstance(game, dict) and 'bookmakers' in game:
                            for bookmaker in game['bookmakers']:
                                for market in bookmaker.get('markets', []):
                                    for outcome in market.get('outcomes', []):
                                        prop = normalize_oddsapi_prop(game, market, outcome, 'MLB')
        ...
        if wnba_props_dir.exists():
            ...
            for prop_file in wnba_files:
                try:
                    with open(prop_file, 'r') as f:
                        game = json.load(f)
                        logger.info(f"Processing WNBA file: {prop_file.name}, data keys: {list(game.keys())}")
                        if isinstance(game, dict) and 'bookmakers' in game:
                            for bookmaker in game['bookmakers']:
                                for market in bookmaker.get('markets', []):
                                    for outcome in market.get('outcomes', []):
                                        prop = normalize_oddsapi_prop(game, market, outcome, 'WNBA')
                                        all_props.append(prop)
                except Exception as e:
                    logger.warning(f"Failed to load WNBA props from {prop_file}: {e}")
        
        logger.info(f"Loaded {len(all_props)} total props from all sports (OddsAPI format)")
        return all_props
    ...
    def _generate_single_ticket_from_props(self, all_props: List[Dict], ticket_number: int) -> Optional[Dict]:
        ...
        try:
            ...
            total_confidence = sum(prop.get('confidence', 0.7) for prop in selected_props) / len(selected_props)
            total_odds = 1.0
            ...
            ticket = {
                ...
                'sport': 'Mixed',  # Mixed sports ticket
                'props': selected_props,
                'total_confidence': round(total_confidence, 3),
                'total_odds': round(total_odds, 3),
                'reasoning': f"High-confidence ticket #{ticket_number} using actual fetched props from multiple sports",
