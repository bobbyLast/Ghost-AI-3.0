...
# Major tournament event types that Underdog offers
MAJOR_EVENT_TYPES = [
    'Atp Singles',      # ATP Tour
    'Wta Singles',      # WTA Tour  
    'Grand Slam',       # Grand Slams
    'Atp Masters 1000', # ATP Masters
    'Wta Premier',      # WTA Premier
    'Wta Premier Mandatory',  # WTA Premier Mandatory
    'Challenger Men Singles',  # Challenger tournaments (Underdog sometimes offers)
    'Challenger Women Singles',  # Challenger tournaments
    'Itf Men Singles',  # ITF tournaments (sometimes on Underdog)
    'Itf Women Singles'  # ITF tournaments
]
...
def get_major_tournament_fixtures(date_start: str, date_stop: Optional[str] = None) -> Any:
    """Get fixtures only from major tournaments that Underdog offers."""
    all_fixtures = get_fixtures(date_start, date_stop)
    ...
    for fixture in all_fixtures:
        ...
        # Only include major tournament types
        if any(major_type in event_type for major_type in MAJOR_EVENT_TYPES):
            # Also filter out doubles matches (Underdog mostly offers singles)
            if 'Doubles' not in event_type:
